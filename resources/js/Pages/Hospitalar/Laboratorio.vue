...
import { 
    ...
    FlaskConical, Check, Box, Microscope, FlaskRound,
    UserCircle, Phone, MapPin, Fingerprint, Dna
...
const selecionarExame = (exame) => {
    ...
    nrAmostra.value = exame.Referencia || '';
};
...
const finalizarLaboratorio = () => {
    if (confirm('Deseja finalizar o atendimento laboratorial para este paciente?')) {
        ...
        <div class="min-h-screen bg-[#f1f5f9] p-4 lg:p-6">
            <div class="max-w-[1800px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6 h-[calc(100vh-120px)]">
                ...
                <div class="lg:col-span-3 flex flex-col gap-6 overflow-hidden">
                    <div class="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-200/60 flex flex-col h-full overflow-hidden">
                        <div class="p-6 border-b border-slate-100 bg-blue-50/30">
                            <h2 class="text-xs font-black text-blue-900 uppercase tracking-widest flex items-center gap-2 mb-6">
                                <Activity class="w-4 h-4 text-blue-600" /> Lista de Espera
                            </h2>
                            <div class="relative">
                ...
                <div class="lg:col-span-9 flex flex-col gap-6 overflow-hidden">
                    <template v-if="selectedPaciente">
                        ...
                        <div class="grid grid-cols-1 xl:grid-cols-12 gap-6 shrink-0">
                            <!-- Patient Info -->
                            <div class="xl:col-span-12 bg-white p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-200/60 grid grid-cols-1 md:grid-cols-4 gap-8">
                                    ...
                                    <div>
                                        ...
                                        <div class="flex flex-wrap items-center gap-4 mt-2">
                                            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                                                <Calendar class="w-3.5 h-3.5" /> {{ details.paciente.DataNascimento }} ({{ calcularIdade(details.paciente.DataNascimento) }})
                                            </span>
                                            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                                                <UserCircle class="w-3.5 h-3.5" /> {{ details.paciente.Genero }}
                                            </span>
                                            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                                                <Phone class="w-3.5 h-3.5" /> {{ details.paciente.Telefone || 'N/D' }}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                <div class="space-y-1">
                                    <p class="text-[10px] font-black text-slate-300 uppercase tracking-widest flex items-center gap-2">
                                        <MapPin class="w-3.5 h-3.5" /> Morada
                                    </p>
                                    <p class="text-xs font-bold text-slate-600 truncate">{{ details.paciente.Morada || 'N/D' }}</p>
                                </div>
                                <div class="flex items-center justify-end gap-3">
                                    <div class="text-right">
                                        <p class="text-[10px] font-black text-slate-300 uppercase tracking-widest">Amostra Nº</p>
                                        <input v-model="nrAmostra" placeholder="Ex: AM-001" class="bg-blue-50 border-transparent focus:bg-white focus:ring-4 focus:ring-blue-500/10 rounded-2xl text-sm font-black text-blue-600 w-32 py-2 text-center" />
                                    </div>
                        ...
                        <div class="flex-grow grid grid-cols-1 xl:grid-cols-12 gap-6 overflow-hidden">
                            ...
                            <div class="xl:col-span-4 bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-200/60 flex flex-col overflow-hidden">
                                <div class="p-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                                    <h3 class="text-[10px] font-black text-slate-800 uppercase tracking-[0.2em] flex items-center gap-2">
                                        <Microscope class="w-4 h-4 text-blue-600" /> Exames Solicitados
                                    </h3>
                                </div>
                                <div class="flex-grow overflow-y-auto p-4 space-y-2 custom-scrollbar">
                                    <div v-for="e in details.exames" :key="e.Id" 
                                        ...
                                        <Beaker v-else class="w-4 h-4 text-blue-500" />
                                    </div>
                                    
                                    <div v-if="details.exames.length === 0" class="p-12 text-center opacity-30">
                                        <FlaskRound class="w-12 h-12 mx-auto mb-4" />
                                        <p class="text-[10px] font-bold uppercase tracking-widest leading-relaxed">Nenhum exame solicitado</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="xl:col-span-8 flex flex-col gap-6">
                                <!-- Results Entry -->
                                <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-200/60 p-8 flex flex-col min-h-[300px]">
                                    <div class="flex items-center justify-between mb-6">
                                        <h3 class="text-[10px] font-black text-slate-800 uppercase tracking-[0.2em] flex items-center gap-2">
                                            <History class="w-4 h-4 text-emerald-600" /> Registro de Resultados
                                        </h3>
                                    ...
                                    <div v-if="selectedExame" class="flex flex-col flex-grow">
                                        <textarea v-model="resultadoText" 
                                                  class="flex-grow w-full bg-slate-50 border-transparent focus:bg-white focus:ring-4 focus:ring-blue-500/10 rounded-[2rem] p-8 text-sm font-medium transition-all resize-none mb-6"
                                                  placeholder="Descreva aqui os resultados analíticos, valores de referência e observações..."></textarea>
                                        
                                        <div class="flex items-center justify-between">
                                            <div class="flex items-center gap-3">
                                                <button class="p-3 bg-slate-100 text-slate-500 rounded-2xl hover:bg-slate-200 transition-all">
                                            ...
                                            <button @click="salvarResultado" class="bg-blue-600 text-white px-10 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-700 transition-all shadow-xl shadow-blue-200 flex items-center gap-2">
                                                <Save class="w-4 h-4" /> Gravar Resultados
                                            </button>
                                        </div>
                                    </div>
                                    <div v-else class="flex-grow flex flex-col items-center justify-center text-center opacity-30">
                                        <Dna class="w-16 h-16 mb-4 text-blue-200" />
                                        <p class="text-xs font-black uppercase tracking-widest">Selecione um exame ao lado</p>
                                    </div>
                                </div>

                                <!-- Action Bar -->
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div class="bg-white rounded-[2rem] p-6 border border-slate-200/60 shadow-sm flex items-center justify-between">
                                        <div class="flex items-center gap-4">
                                            <div class="w-12 h-12 bg-amber-50 text-amber-600 rounded-2xl flex items-center justify-center">
                                                <Box class="w-6 h-6" />
                                            </div>
                                            <div>
                                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Materiais Usados</p>
                                                <p class="text-sm font-black text-slate-800">0 Itens Registrados</p>
                                            </div>
                                        </div>
                                        <button class="bg-slate-900 text-white p-3 rounded-xl hover:bg-slate-800 transition-all">
                                            <Plus class="w-5 h-5" />
                                        </button>
                                    </div>

                                    <div class="bg-emerald-600 rounded-[2rem] p-4 shadow-xl shadow-emerald-200 flex">
                                        <button @click="finalizarLaboratorio" class="w-full bg-white text-emerald-600 rounded-2xl text-xs font-black uppercase tracking-[0.3em] hover:bg-emerald-50 transition-all flex items-center justify-center gap-4 py-4">
                                            <CheckCircle class="w-6 h-6" /> FINALIZAR PROCESSO
                                        </button>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div v-else class="flex-grow flex flex-col items-center justify-center bg-white rounded-[3rem] border-2 border-dashed border-slate-200 p-12 text-center">
                        <div class="w-32 h-32 bg-blue-50 rounded-full flex items-center justify-center mb-8">
                            <Beaker class="w-12 h-12 text-blue-200" />
                        </div>
                        <h2 class="text-xl font-black text-slate-300 uppercase tracking-[0.3em]">Laboratório de Análises</h2>
                        <p class="text-sm font-bold text-slate-400 mt-4 max-w-sm leading-relaxed">Inicie o processamento laboratorial selecionando um paciente da lista de espera ao lado.</p>
                    </div>
                </div>
            </div>
        </div>
