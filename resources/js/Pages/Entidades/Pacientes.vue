...
const confirmDelete = () => {
    ...
    itemToDelete.value = selectedPacientes.value[0];
    isDeleteModalOpen.value = true;
};

const executeDelete = () => {
    if (itemToDelete.value) {
        form.delete(route('pacientes.destroy', itemToDelete.value), {
            onSuccess: () => {
    ...
    <DashboardLayout>
        ...
        <div class="bg-white rounded-t-2xl border border-gray-100 p-3 shadow-sm flex items-center justify-between">
            <div class="flex items-center space-x-1 overflow-x-auto custom-scrollbar">
                ...
                <button @click="confirmDelete" :class="{'opacity-50 cursor-not-allowed': selectedPacientes.length !== 1}" class="flex items-center px-4 py-2 text-xs font-bold text-gray-700 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors border border-transparent hover:border-red-100">
        ...
        <div v-if="isDeleteModalOpen" class="fixed inset-0 z-[60] flex items-center justify-center bg-gray-900/60 backdrop-blur-md p-4 animate-fadeIn">
            <div class="bg-white rounded-2xl shadow-2xl border border-gray-100 w-full max-w-md overflow-hidden transform transition-all scale-100">
                <div class="p-6 text-center">
                    ...
                    <p class="text-sm text-gray-500 mb-6">Tem a certeza que deseja eliminar o paciente selecionado? Esta ação não pode ser desfeita.</p>
